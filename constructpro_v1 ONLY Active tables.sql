...
CREATE TABLE project_settlements (
    ...
    settlement_type ENUM(
        'advance_to_cost'
    ) NOT NULL DEFAULT 'advance_to_cost',

    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,

    PRIMARY KEY (id),

    CONSTRAINT fk_project_settlements_project
        FOREIGN KEY (project_id)
        REFERENCES projects(id)
        ON DELETE RESTRICT
        ON UPDATE CASCADE,

    CONSTRAINT fk_project_settlements_advance
        FOREIGN KEY (advance_id)
        REFERENCES project_advances(id)
        ON DELETE RESTRICT
        ON UPDATE CASCADE,

    CONSTRAINT fk_project_settlements_cost
        FOREIGN KEY (cost_id)
        REFERENCES project_costs(id)
        ON DELETE RESTRICT
        ON UPDATE CASCADE,

    CONSTRAINT chk_project_settlements_amount
        CHECK (amount > 0),

    INDEX idx_project_settlements_project (project_id),
    INDEX idx_project_settlements_advance (advance_id),
    INDEX idx_project_settlements_cost (cost_id),

    UNIQUE KEY uq_project_settlements_advance_cost_type
        (advance_id, cost_id, settlement_type)
